...
namespace kvdb::endian
{
    ...
    )
    {
        ...
        put_u32_le(out, static_cast<uint32_t>(bytes.size()));
    ...
    )
    {
        ...
        if (!write_u32_le(file, static_cast<uint32_t>(bytes.size())))
        {
            return false;
        }
    ...
    std::optional<uint16_t> read_u16_le(std::ifstream& file)
    {
        ...
        value |= static_cast<uint16_t>(bytes[0]) << 0;
    ...
    std::optional<uint64_t> read_u64_le(std::ifstream& file)
    {
        ...
        value |= static_cast<uint64_t>(bytes[0]) << 0;
        value |= static_cast<uint64_t>(bytes[1]) << 8;
        value |= static_cast<uint64_t>(bytes[2]) << 16;
        value |= static_cast<uint64_t>(bytes[3]) << 24;
        value |= static_cast<uint64_t>(bytes[4]) << 32;
        value |= static_cast<uint64_t>(bytes[5]) << 40;
        value |= static_cast<uint64_t>(bytes[6]) << 48;
        value |= static_cast<uint64_t>(bytes[7]) << 56;
    ...
    std::optional<uint8_t> Reader::read_u8()
    {
        if (remaining() < 1)
        {
            return std::nullopt;
        ...
        uint8_t value = static_cast<uint8_t>(data_[pos_]);
        ++pos_;

        return value;
    }

    std::optional<uint16_t> Reader::read_u16_le()
    {
        if (remaining() < 2)
        {
            return std::nullopt;
        ...
        value |= static_cast<uint16_t>(data_[pos_ + 0]) << 0;
        value |= static_cast<uint16_t>(data_[pos_ + 1]) << 8;
    ...
    std::optional<uint32_t> Reader::read_u32_le()
    {
        if (remaining() < 4)
        {
            return std::nullopt;
        ...
        value |= static_cast<uint32_t>(data_[pos_ + 0]) << 0;
        value |= static_cast<uint32_t>(data_[pos_ + 1]) << 8;
        value |= static_cast<uint32_t>(data_[pos_ + 2]) << 16;
        value |= static_cast<uint32_t>(data_[pos_ + 3]) << 24;
    ...
    std::optional<uint64_t> Reader::read_u64_le()
    {
        if (remaining() < 8)
        {
            return std::nullopt;
        ...
        value |= static_cast<uint64_t>(data_[pos_ + 0]) << 0;
        value |= static_cast<uint64_t>(data_[pos_ + 1]) << 8;
        value |= static_cast<uint64_t>(data_[pos_ + 2]) << 16;
        value |= static_cast<uint64_t>(data_[pos_ + 3]) << 24;
        value |= static_cast<uint64_t>(data_[pos_ + 4]) << 32;
        value |= static_cast<uint64_t>(data_[pos_ + 5]) << 40;
        value |= static_cast<uint64_t>(data_[pos_ + 6]) << 48;
        value |= static_cast<uint64_t>(data_[pos_ + 7]) << 56;
    ...
    std::optional<std::span<const std::byte>> Reader::read_bytes(uint32_t size)
    {
        if (remaining() < size)
        {
            return std::nullopt;
